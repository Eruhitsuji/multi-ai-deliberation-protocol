...
def main() -> int:
    ...
    for rel in FORBIDDEN_STAGING_PATHS:
        if (ROOT / rel).exists():
            problems.append(f'forbidden staging path remains in release source: {rel}')

    allowed = {item for item in args.allow_blockers.split(',') if item}
    status = yaml.safe_load(
        (ROOT / 'docs/planning/MADP-v0.3.0-alpha.3-implementation-status.yaml').read_text(encoding='utf-8')
    )
    if status.get('protocol_version') != 'MADP-v0.3.0-alpha.3':
        problems.append('protocol version mismatch')
    if status.get('implementation_status') != 'RELEASE_CANDIDATE_CONTENT_READY':
        problems.append('content status mismatch')
    policy = status.get('validation_policy', {})
    if (
        policy.get('self_attestation_is_release_evidence') is not False
    ...
    if 'automated_checks' in status:
        problems.append('handwritten automated_checks map cannot be release evidence')

    blockers = {item['id']: item['status'] for item in status.get('release_blockers', [])}
    if set(blockers) != KNOWN:
        problems.append('release blocker set mismatch')
    active = {key for key, value in blockers.items() if value != 'DONE'}
    if active != allowed:
        problems.append(f'unexpected active blockers: {sorted(active)} expected {sorted(allowed)}')
    ...
    if not evidence_path.is_file():
        ...
        if (
            evidence.get('evidence_version') != 'MADP-ALPHA3-VALIDATION-EVIDENCE-v1'
        ...
        if evidence.get('repository_commit') != args.repository_commit:
            problems.append('evidence commit mismatch')
        if bool(evidence.get('release_mode')) != bool(args.require_main):
            problems.append('evidence release mode mismatch')
        scope = evidence.get('scope_sha256', {})
        if not scope:
            problems.append('evidence scope hash set missing')
        for rel, digest in scope.items():
            source_path = ROOT / rel
            if not source_path.is_file() or sha(source_path) != digest:
                problems.append(f'evidence scope hash mismatch: {rel}')
        rows = {item.get('id'): item for item in evidence.get('checks', [])}
        if set(rows) != REQUIRED_CHECKS:
            problems.append('evidence check set mismatch')
        for check_id, row in rows.items():
            checker_path = ROOT / row.get('checker_path', '')
            if row.get('status') != 'PASS' or row.get('return_code') != 0:
                problems.append(f'evidence check failed: {check_id}')
            if not checker_path.is_file() or sha(checker_path) != row.get('checker_sha256'):
                problems.append(f'checker hash mismatch: {check_id}')
            if hashlib.sha256((row.get('stdout') or '').encode()).hexdigest() != row.get('stdout_sha256'):
                problems.append(f'stdout evidence hash mismatch: {check_id}')
            if hashlib.sha256((row.get('stderr') or '').encode()).hexdigest() != row.get('stderr_sha256'):
                problems.append(f'stderr evidence hash mismatch: {check_id}')
            for rel, digest in row.get('input_sha256', {}).items():
                input_path = ROOT / rel
                if not input_path.is_file() or sha(input_path) != digest:
                    problems.append(f'input hash mismatch: {check_id}:{rel}')
    ...
    if not release_a.is_dir() or not release_b.is_dir():
        ...
        if tree_hashes(release_a) != tree_hashes(release_b):
            problems.append('release artifact generation is not reproducible')
        bound = release_a / 'validation-evidence.yaml'
        if not bound.is_file() or not evidence_path.is_file() or sha(bound) != sha(evidence_path):
            problems.append('release artifact is not bound to supplied evidence')
        process = subprocess.run(
            [
                sys.executable,
                str(ROOT / 'scripts/check_generated_alpha3_release_artifacts.py'),
                str(release_a),
                '--source-commit',
    ...
    if args.require_main:
        ...
        if status.get('release_ready') is not False:
            problems.append('branch must not claim release_ready')
