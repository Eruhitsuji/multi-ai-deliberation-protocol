...
def _with_generated_metadata(text: str, source_path: str, source_repository: str, source_commit: str) -> str:
    comment = f"<!-- Generated from {source_repository} at {source_commit}; source template: {source_path}. -->"
...
def _index_html(owner: str, repository: str, source_commit: str) -> str:
    ...
    links = [
        ...
        ("Use commands", "bootstrap/use-madp-commands.md"),
        ("Share context", "bootstrap/share-context-with-ai.md"),
        ("Request review", "bootstrap/request-review.md"),
        ("AI-driven development", "bootstrap/use-madp-for-ai-driven-development.md"),
        ("Complete protocol bundle", BUNDLE_OUTPUT_PATH),
        ("Bundle manifest", BUNDLE_MANIFEST_OUTPUT_PATH),
        ("Manifest", "bootstrap/manifest.yaml"),
        ("Source repository", repo_url),
        ("Source commit", f"{repo_url}/commit/{source_commit}"),
    ]
    items = "\n".join(f'      <li><a href="{escape(href, quote=True)}">{escape(label)}</a></li>' for label, href in links)
    return f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{PROTOCOL_VERSION} Bootstrap Prompts</title></head>
<body><main><h1>{PROTOCOL_VERSION} Bootstrap Prompts</h1><p>Protocol version: <code>{PROTOCOL_VERSION}</code></p><p>Generated from {escape(owner)}/{escape(repository)} at commit <code>{escape(source_commit)}</code>.</p><ul>
{items}
</ul></main></body></html>
"""
...
def generate(output_dir: Path, source_repository: str, source_sha: str, workflow_run_id: str, generated_by: str) -> None:
    ...
    for name in BOOTSTRAP_FILES:
        ...
        generated = _replace_repository_placeholders(source.read_text(encoding="utf-8"), owner, repository, source_commit)
        generated = _with_generated_metadata(generated, rel(source), source_repository, source_commit)
        target = bootstrap_output / name
        _write_text(target, generated)
        generated_files.append({"path": f"bootstrap/{name}", "source_template": rel(source), "source_commit": source_commit, "sha256": _sha256(generated)})
    ...
    bundle_manifest = yaml.safe_dump({"complete_protocol_bundle": {
        ...
        "files": [{"path": path, "sha256": _sha256(_source_file_text(path))} for path in CANONICAL_BUNDLE_FILES],
    ...
    generated_files.extend([
        {"path": BUNDLE_OUTPUT_PATH, "source_files": CANONICAL_BUNDLE_FILES, "source_commit": source_commit, "sha256": _sha256(bundle)},
        {"path": BUNDLE_MANIFEST_OUTPUT_PATH, "source_files": CANONICAL_BUNDLE_FILES, "source_commit": source_commit, "sha256": _sha256(bundle_manifest)},
    ])
    manifest = yaml.safe_dump({"generated_bootstrap": {
        "protocol_version": PROTOCOL_VERSION,
        "source_repository": source_repository,
        "source_commit": source_commit,
        "generated_by": generated_by,
        "workflow_run_id": workflow_run_id,
        "status": "published prerelease implementation aid",
