...
LOAD_REPORT_VERSION='MADP-PROTOCOL-LOAD-REPORT-v1'
...
def main():
 ...
 if manual.get('protocol_load_report_required') is not True or manual.get('required_load_report_version')!=LOAD_REPORT_VERSION:problems.append('manual trial protocol load requirement missing')
 if a.release:
  m=manual.get('metrics',{});rows=manual.get('scenario_results',[])
  if manual.get('status')!='PASS':problems.append('manual trials not PASS')
  if {x.get('scenario_id') for x in rows}!=REQUIRED:problems.append('release trial results must cover every usability scenario exactly')
  if len(rows)!=len(REQUIRED):problems.append('release trial results contain duplicate or extra scenario records')
  for row in rows:
   tested=row.get('tested_commit');report=row.get('protocol_load_report',{})
   if not valid_commit(tested):problems.append(f'invalid tested commit: {row.get("scenario_id")}')
   if report.get('report_version')!=LOAD_REPORT_VERSION or report.get('protocol_version')!='MADP-v0.3.0-alpha.3':problems.append(f'invalid protocol load report: {row.get("scenario_id")}')
   if report.get('status')!='COMPLETE' or report.get('all_required_files_read') is not True or report.get('inferred_unread_content') is not False:problems.append(f'incomplete protocol load gate: {row.get("scenario_id")}')
   if report.get('repository_commit')!=tested:problems.append(f'protocol load report commit mismatch: {row.get("scenario_id")}')
